...
class ResumenIngresosEgresosController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $egresos = $this->getEgresos($request, auth()->user());
    ...
    public function generarPdf(Request $request)
    {
        ...
        $egresos = $this->getEgresos($request, $usuario);
        $resumen = $this->getResumen($request, $ingresos, $egresos, $usuario);
    ...
    private function getIngresos($request, User $usuario)
    {
        ...
        return ItComprobantePago::whereDate('cp_fecha_cobro', '>=', $fechaInicial)
            ...
            ->where('us_codigo', $usuario->us_codigo)
            ->orderBy('cp_fecha_cobro', 'asc')
            ->get();
    }

    private function getEgresos($request, User $usuario)
    {
        $fechaInicial = $request->fechaInicial;
        $fechaFinal = $request->fechaFinal;
