$(document).ready(function () {
    ...
    function loadProgramacion(page, term) {
        $.ajax({
            type: "GET",
            url: BASEURL + "/admin_servicios/registro-servicios?page=" + page + "&term=" + term,
            headers: {
        ...
        let html = `<tr>
                        ...
                        <td>
                            <button class="btn btn-secondary btn-sm pago" 
                                ...
                                data-servicio="${matricula.servicio}"
                                title="pago cuota"
                                ${disabled}
                                >
    ...
    $("#btnEntregar").click(function (e) {
        $.ajax({
            type: "PATCH",
            url: BASEURL + "/admin_servicios/entregar-certificado/" + codigoProgramacion,
            headers: {
                Accept: "application/json",
    ...
    $(document).on("click", ".pago", function (e) {
        ...
        $("#servicioDescripcion").html(servicio);
    ...
    $("#btnPago").click(function (e) {
        ...
        $.ajax({
            type: "PUT",
            url: BASEURL + "/admin_servicios/pago-cuota/" + codigoProgramacion,
            headers: {
                Accept: "application/json",
                Authorization: "Bearer " + token, // Pasar el token como parte de la cabecera de autorización
            },
    ...
    $("#btnRegistrar").click(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: BASEURL + "/admin_servicios/registro-servicios",
            headers: {
                Accept: "application/json",
                Authorization: "Bearer " + token, // Pasar el token como parte de la cabecera de autorización
            },
    ...
    importe.addEventListener("input", (e) => {
        ...
        let costoCalcular = parseFloat(costo.value) || 0;
        if (importe < costoCalcular) {
            saldo.value = costoCalcular - importe;
        } else {
            e.target.value = costoCalcular;
            saldo.value = 0;
        }
    })
});
