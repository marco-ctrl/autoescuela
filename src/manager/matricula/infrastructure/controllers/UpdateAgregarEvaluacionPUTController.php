...
use Src\manager\matricula\infrastructure\validators\UpdateAgregarEvaluacionValidatorRequest;
use Symfony\Component\HttpFoundation\Response;

final class UpdateAgregarEvaluacionPUTController extends Controller
{
    public function index(ItMatricula $matricula, UpdateAgregarEvaluacionValidatorRequest $request): JsonResponse
    {
        try {
            $matriculas = ItMatricula::find($matricula->ma_codigo);

            if($matriculas->ma_evaluacion == 1){
                return response()->json([
                    'status' => false,
                    'message' => __('No se puede adicionar una evaluacion para este numero de matricula porque ya se registro una evaluacion'),
                ], Response::HTTP_OK);
            }
            
            $matriculas->ma_costo_evaluacion += $request->costo;
            $matriculas->ma_costo_total += $request->costo;
            $matriculas->ma_evaluacion = 1;
            $matriculas->save();
            ...
            $data = [
                ...
                "descripcion" => 'ADICION EVALUACION MATRICULA NRO. '. $matriculas->ma_codigo .'/' . "CUOTA N°-" . $cuota->ct_numero,
            ...
            return response()->json([
                ...
                'message' => __('Evaluacion agregada correctamente'),
